...
import {
  ...
  calculerCotisationSalariale,
  calculerCotisationPatronale,
} from "./cotisation.service.js";
...
export const creerDeclaration = (
  ...
  for (let ligne of salaires) {
    if (!ligne.assureId) {
      return { erreur: "assuree requis pour chaque salaire" };
    }
  ...
  const existe = declarations.find(
    (d) => d.employeurId === employeurId && d.month === mois,
  );
  if (existe) {
...
export const calculerTotalDeclaration = (declarationId) => {
  ...
  const montantCotisations = declaration.salaries.reduce(
    (sum, ligne) => sum + calculerCotisationTotale(ligne.montant),
    0,
  );
...
export const genererRecapitulatif = (declarationId) => {
  ...
  const montantTotal = detailsSalaries.reduce(
    (sum, d) => sum + d.cotisationTotale,
    0,
  );
...
export const obtenirTotalParEmployeur = (employeurId) => {
  const declarationsEmployeur = declarations.filter(
    (d) => d.employeurId === employeurId,
  );

  if (declarationsEmployeur.length === 0) return 0;

  return declarationsEmployeur.reduce((total, decl) => {
    const montantCotisations = decl.salaries.reduce(
      (sum, ligne) => sum + calculerCotisationTotale(ligne.montant),
      0,
    );
    const resultat = calculerTotalAvecPenalite(
